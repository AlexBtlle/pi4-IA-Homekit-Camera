...
| `camera.day_gamma` | 1.0 | **(bêta)** Éclaircissement **automatique** des scènes **couleur** sombres — une courbe gamma sur la luma (relève les ombres, préserve les hautes lumières), en couleur. Activé **uniquement quand la scène est sombre** (estimation Lux de l'AEC sous le seuil), avec hystérésis ; de jour le déclencheur reste inactif et l'image n'est pas touchée — aucune surexposition d'une pièce ensoleillée. À cette lumière le capteur est déjà saturé : c'est un lift numérique (plus clair mais plus bruité ; le denoise ISP nettoie). La nuit/IR a son propre chemin (`ir_grayscale`). Plus haut = ombres plus claires ; 2.5 testé sur le terrain (pièce sombre au crépuscule), 1.0 = off. |
