...
| `camera.day_ev` | 0.0 | **(beta)** Automatic brightening for dim **colour** scenes, in EV/stops (+1 = 2× the AE target). Engaged **only when the scene is dark** (the sensor pins its analogue gain), with hysteresis; in daylight the trigger stays off and the image is untouched — a sunny room is never over-exposed. Where it kicks in the sensor is already pinned, so it lifts via ISP digital gain (brighter but noisier). Night/IR uses its own path (`ir_grayscale`). Useful range 0.0–1.5; 0.0 = off. |
