...
| `camera.day_ev` | 0.0 | **(beta)** Daytime/colour exposure bias in EV/stops (+1 = 2× the AE target), to brighten dim colour scenes. In low light the sensor is already pinned (gain 8×), so a positive bias lifts via ISP digital gain — brighter but noisier. Night/IR uses its own path (`ir_grayscale`). Useful range 0.0–1.5; 0.0 = off. |
